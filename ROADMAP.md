...
- [x] Agente Ricercatore — **multi-database con deduplica** FATTO. Fonti gratuite collegate (giu 2026): **PubMed, Europe PMC, ClinicalTrials.gov, OpenAlex, Semantic Scholar** (registro fonti robusto: se una è giù/rate-limited viene marcata "non disponibile" senza bloccare le altre; query stile PubMed ridotta a parole chiave per i motori non-booleani). Pagina "Ricerca" live: conteggi per database = record identificati PRISMA, candidati unici dopo dedup. Note: **CrossRef** collegato ma fuori dal default (conteggio fuzzy ~150k, non PRISMA-sensato; utile per risoluzione DOI); **Semantic Scholar** flaky senza chiave (429) → eventuale chiave gratuita lo renderebbe stabile. Da aggiungere ancora: WHO ICTRP, scoring affidabilità; **Embase** richiede licenza a pagamento (gold standard mancante).
